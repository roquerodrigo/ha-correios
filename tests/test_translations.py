...
TRANSLATIONS_DIR = (
    Path(__file__).parent.parent / "custom_components" / "correios" / "translations"
)
...
def _authored_entity_keys() -> set[tuple[str, str]]:
    """Retorna todos os pares `(platform, key)` que o `en.json` define em `entity`."""
...
async def test_entity_translation_keys_and_authored_names_agree(
    ...
    """
    Toda translation key que uma entidade pede está escrita, e vice-versa.

    A leitura vem do entity registry, e não das classes: o Home Assistant
    reescreve `_attr_translation_key` como property, então inspecionar o
    atributo da classe encontra um descriptor e silenciosamente não confere
    nada. Comparar os locales entre si também não pega isso — uma chave
    ausente dos dois lados coincide, e a entidade recorre ao nome da sua
    device class enquanto o nome escrito nunca aparece.
    """
    ...
    requested = {
        ...
        if entry.platform == "correios" and entry.translation_key is not None
    }
