...
async def test_options_flow_persists_scan_interval(hass, setup_integration):
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        user_input={CONF_SCAN_INTERVAL: 600, "delivered_retention_days": 3},
    )
    assert result["type"] == FlowResultType.CREATE_ENTRY
    assert setup_integration.options[CONF_SCAN_INTERVAL] == 600
    assert setup_integration.options["delivered_retention_days"] == 3


async def test_options_flow_uses_existing_value_as_default(hass, setup_integration):
    hass.config_entries.async_update_entry(
        setup_integration,
        options={CONF_SCAN_INTERVAL: 1200, "delivered_retention_days": 2},
    )
    ...
    assert scan_interval_key.default() == 1200
    retention_key = next(
        k for k in schema if getattr(k, "schema", k) == "delivered_retention_days"
    )
    assert retention_key.default() == 2
