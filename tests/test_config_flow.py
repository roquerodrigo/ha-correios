...
def _patch_client(side_effect=None, return_value=None):
    ...
    return patch("custom_components.correios.config_flow.CorreiosApiClient")
...
async def test_step_user_success_creates_entry(hass, enable_custom_integrations):
    with _patch_client() as mock:
        mock.return_value.async_authenticate = AsyncMock(return_value={"title": "ok"})
        result = await _start_user_flow(hass)
...
async def test_step_user_auth_error_shows_auth(hass, enable_custom_integrations):
    with _patch_client() as mock:
        mock.return_value.async_authenticate = AsyncMock(
            side_effect=CorreiosApiClientAuthenticationError("bad")
        )
        flow = await _start_user_flow(hass)
        result = await hass.config_entries.flow.async_configure(
            flow["flow_id"], user_input=USER_INPUT
...
async def test_step_user_communication_error_shows_connection(
    ...
    with _patch_client() as mock:
        mock.return_value.async_authenticate = AsyncMock(
            side_effect=CorreiosApiClientCommunicationError("down")
        )
        flow = await _start_user_flow(hass)
        result = await hass.config_entries.flow.async_configure(
            flow["flow_id"], user_input=USER_INPUT
...
async def test_step_user_generic_error_shows_unknown(hass, enable_custom_integrations):
    with _patch_client() as mock:
        mock.return_value.async_authenticate = AsyncMock(
            side_effect=CorreiosApiClientError("oops")
        )
        flow = await _start_user_flow(hass)
        result = await hass.config_entries.flow.async_configure(
            flow["flow_id"], user_input=USER_INPUT
...
async def test_reauth_auth_error_shows_auth(hass, enable_custom_integrations):
    ...
    with _patch_client() as mock:
        mock.return_value.async_authenticate = AsyncMock(
            side_effect=CorreiosApiClientAuthenticationError("nope")
        )
        result = await entry.start_reauth_flow(hass)
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"], user_input=NEW_INPUT
...
async def test_step_user_masks_a_cpf_in_the_title(hass, enable_custom_integrations):
    with _patch_client() as mock:
        mock.return_value.async_authenticate = AsyncMock(return_value=None)
        result = await _start_user_flow(hass)
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            user_input={"username": " 123.456.789-09 ", "password": " pass "},
        )
    assert result["type"] == FlowResultType.CREATE_ENTRY
    assert result["title"] == "***.456.789-**"
    assert result["data"] == {"username": "123.456.789-09", "password": " pass "}
