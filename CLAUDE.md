...
- O `sensor.py` adiciona um `CorreiosPackageSensor` para cada código de rastreamento novo e remove do entity registry as entradas dos pacotes que saíram do payload — a cada atualização do coordinator e uma vez no setup, o que também limpa sobras de quando o Home Assistant esteve fora do ar.
